...
type Client struct {
	...
	// The org the clients wants to receive updates for
	organization string
}
...
func extractOrg(tokenStr string) (string) {
	...
	user_id := session["identity"].(map[string]interface{})["id"].(string)

	return getOrganizationId(user_id)
}
